...
class ConstantSignalModel:
    ...
    def predict(self, X):
        return np.full(len(X), self.predicted_label)

    def predict_proba(self, X):
        return np.tile([0.05, 0.10, 0.85], (len(X), 1))
...
class WalkForwardTests(unittest.TestCase):
    ...
    def test_run_walk_forward_backtest_combines_predictions_metrics_and_backtest(self):
        df = pd.DataFrame(
            {
                ...
                "target_signal_label": [2] * 10,
                "target_signal": ["BUY"] * 10,
            }
        )
        train_calls = []
        ...
        self.assertTrue((train_calls[0][1] == 2).all())
        self.assertEqual(4, len(predictions_df))
        self.assertEqual([1, 2], fold_metrics_df["fold_id"].tolist())
        self.assertIn("Accuracy", fold_metrics_df.columns)
        self.assertIn("predicted_signal", predictions_df.columns)
